...
% }
% if not {
        <a href="/random" title="Browse" %(`{if(~ $req_path /user/* || ~ $req_path /random && ! ~ $req_path /user/$logged_user) echo 'class="active"'}%)><i class="mdi mdi-magnify"></i></a><a href="/matches" title="Matches" %(`{if(~ $req_path /matches) echo 'class="active"'}%)><i class="mdi mdi-heart"></i></a><a href="/user/%($logged_user%)" title="Profile" %(`{if(~ $req_path /user/$logged_user || ~ $req_path /edit || ~ $req_path /settings) echo 'class="active"'}%)><i class="mdi mdi-account-card-details"></i></a><a href="/premium" title="Get Premium" %(`{if(~ $req_path /premium) echo 'class="active"'}%)><i class="mdi mdi-star"></i></a>%(`{if(check_user admin || check_user mod) echo '<a href="/admin" title="Admin"><i class="mdi mdi-gavel"></i></a>'}%)
% }
    </div>
</nav>
<main role="main">
